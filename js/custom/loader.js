window.onload = function () {
    queue()
    .defer(d3.json, "data/VIC.json")
    .defer(d3.csv, "data/CrimeType.csv")
    .defer(d3.csv, "data/CUZscore.csv")
    .defer(d3.csv, "data/HappinessCrime.csv")
    .await(function (error, mapData, crimeTypeData, crimeUnemploymentData, happinessCrimeData) {
        if (error) {
            return console.error(error);
        }
        var g = drawMap(mapData, crimeUnemploymentData);
        fillMap(g, crimeTypeData);
        drawSunburst(crimeTypeData);
        drawInteractiveBarChart(happinessCrimeData);
